...
\newpage
#### TABLE 5. STARS model simulations for route-specific entrainment, travel times, and survival
...
```{r, echo = FALSE}
table_stars_w <- tidyr::pivot_wider(table_stars, names_from = "Route", values_from  = "Estimate", id_cols = c("Stock", "Date", "Parameter")) %>%
  select(Stock, Date, Parameter, `Delta Cross Channel`, `Georgiana Slough`, `Sutter and Steamboat Slough`, `Interior Delta`)

flextable(table_stars_w) %>%
  vline()%>%
  hline() %>%
  border_outer() %>%
  width(width = c(1.4, 1, 0.8, 0.8, 0.8, 0.8, 0.8))%>%
  bold(part = "header")
```
\newpage
#### TABLE 6. a) WY 2022 loss and salvage predictor data: Predicted weekly loss of winter-run Chinook salmon and steelhead at CVP and SWP facilities. 
...
#### TABLE 6. b) Environmental details, current and forecast.
```{r, echo = FALSE}
